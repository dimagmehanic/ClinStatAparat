...
```{r, echo=FALSE}
#ADVS data set prep
advs  <- adam$advs() %>% filter( SAFFL == "Y" & AVISIT == "Baseline" & DTYPE == "AVERAGE" ) %>% 
  select(USUBJID, AGE, SEX, TRT01P, PARAM, PARAMCD, AVAL ) %>%
  pivot_wider(
    id_cols = c(USUBJID, AGE, SEX, TRT01P),
    names_from = PARAMCD,
    values_from = c(AVAL)
  )
...
#ADLB data prep
adlb <- adam$adlb() %>% 
  filter(SAFFL == "Y" &  AVISITN==9999 & ANL01FL == "Y" & 
           PARAMCD %in% c("ALB", "ALT", "CA", "CHOLES", "AST", "HGB", "RBC", "WBC", "PH")) %>%  
...
anl <- left_join(advs, adlb, by = c("USUBJID")) 

A <-  anl %>%
  filter(!is.na(AVAL_CA)) %>%
  pull(AVAL_CA)

B <-  A - na.omit(anl$BASE_CA)
...
**Variable A**
  ```{r}
  get_quartiles(A, na.rm=TRUE) %>% df2print() 
  ```
  </div> 
...
**Variable B**
  ```{r}
  get_quartiles(B, na.rm=TRUE) %>% df2print()
  ```
  </div>
</div>
